package Graph;

import java.util.*;
import java.io.*;
import java.lang.*;


public class NoOfDependency{ 
	// A user define class to represent a graph. 
	// A graph is an array of adjacency lists. 
	// Size of array will be V (number of vertices 
	// in graph) 
	static class Graph 
	{ 
		int V; 
		LinkedList<Integer> adjListArray[]; 
		
		// constructor 
		Graph(int V) 
		{ 
			this.V = V; 
			
			// define the size of array as 
			// number of vertices 
			adjListArray = new LinkedList[V]; 
			
			// Create a new list for each vertex 
			// such that adjacent nodes can be stored 
			for(int i = 0; i < V ; i++){ 
				adjListArray[i] = new LinkedList<>(); 
			} 
		} 
	} 
	
	// Adds an edge to an undirected graph 
	static void addEdge(Graph graph, int src, int dest) 
	{ 
		// Add an edge from src to dest. 
		graph.adjListArray[src].add(dest); 
		
		// Since graph is undirected, add an edge from dest 
		// to src also
		
		//aph.adjListArray[dest].add(src); 
	} 
	
	// A utility function to print the adjacency list 
	// representation of graph 
	static void printGraph(Graph graph) 
	{	 int count=0;
		for(int v = 0; v < graph.V; v++) 
		{  
//	System.out.print(v);  
			for(Integer pCrawl: graph.adjListArray[v]){ 
				if(pCrawl!=null){count++;}
//		System.out.print(" -> "+pCrawl); 
			}
//	System.out.println("\n");
		} System.out.println(count);
	} 
	
/*/ Driver program to test above functions 
	public static void main(String args[]) 
	{ 
		// create the graph given in above figure 
		int V = 5; 
		Graph graph = new Graph(V); 
		addEdge(graph, 0, 1); 
		addEdge(graph, 0, 4); 
		addEdge(graph, 1, 2); 
		addEdge(graph, 1, 3); 
		addEdge(graph, 1, 4); 
		addEdge(graph, 2, 3);  
		addEdge(graph, 3, 4); 
	
		// print the adjacency list representation of 
		// the above graph 
		printGraph(graph); 
	} 
} */
	
	 public static void main(String [] args)throws IOException{
    //inkedList<Integer>l= new  LinkedList<>();
	//inkedHashMap<Integer,LinkedList> adjList=new LinkedHashMap<>();
    //LinkedList<String> clist=new LinkedList<String>();
   // LinkedList<String> oplist=new LinkedList<>();
    //LinkedList<String> op;
    //HashMap<String,Integer>visited=new HashMap<>();
	 BufferedReader br = new BufferedReader(new InputStreamReader(System.in));
     int t = Integer.parseInt(br.readLine());
     for(int i=0; i<t; i++) {
    	 String[] input = br.readLine().split("\\s+");
    	 int e =  Integer.parseInt(input[0]);
         int f = Integer.parseInt(input[1]);
 		Graph graph = new Graph(f);
    	 String []  input1 = br.readLine().split("\\s+");
   //  System.out.println("hi"+input1.length);
     for(int l1 = 0 ;l1 < (input1.length);) {
    	 Integer a=Integer.parseInt(input1[l1]);
    	 Integer b=Integer.parseInt(input1[l1+1]);
    	 l1=l1+2;
    	 addEdge(graph, a,b);
     }	printGraph(graph); 
     }
     
}
}
